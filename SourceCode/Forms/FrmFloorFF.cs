...
namespace TMCAnalyzer {
	public partial class FrmFloorFF:Form {
        ...
        private void AxisFBgain_0_AfterChangeValue(object sender, NationalInstruments.UI.AfterChangeNumericValueEventArgs e) {
            var numFBgain = (NumericEdit)sender;
            if (numFBgain.Value == 0) // show zero as "0", not as "0.000E+000"
                numFBgain.FormatMode = NationalInstruments.UI.NumericFormatMode.CreateSimpleDoubleMode(0);  // example: " 0 "
            else
                numFBgain.FormatMode = NationalInstruments.UI.NumericFormatMode.CreateSimpleDoubleMode(2);  // example: "10.32"
        }
